/*!
  @defgroup rail_b Механизм шины, реализация B
  @ingroup rail
  @{
  Данная реализация целиком базируется на использовании стандартных механизмов
  POSIX IPC.
  Для синхронизации очередей используются блокировки rwlocks с приоритетом читателя (умолчательная стратегия в Linux).
  Для предотвращения старвации писателя используется дополнительная исключающая блокировка mutex, захватываемая
  на время ожидания и получения блокировки rwlock с повышением приоритета писателя.
*/
...
/*!
  ...
  С помощью данной структуры вызовы блокировок реализуют механизм чтения/записи с
  приоритетом записывающего.
*/
typedef struct {
  pthread_mutex_t wlock;    //!< мьютекс для захвта писателем.
  pthread_rwlock_t rwlock;  /*!< общий мьютекс допускающий неэксклюзивную по
                                 чтению блокировку */
} SQRPrwLock_t;
...
int sqrp_lock_init ( SQRPrwLock_t* lock )
{
  ...
  EC_RC ( pthread_mutexattr_init ( &mattr ) );
  state = 1;
  EC_RC ( pthread_mutexattr_setpshared ( &mattr, PTHREAD_PROCESS_SHARED ) );
  ...
  EC_RC ( pthread_mutex_init ( &lock->wlock, &mattr ) );
  state = 3;
  EC_RC ( pthread_rwlock_init ( &lock->rwlock, &rwattr ) );
  state = 4;

  EC_RC ( pthread_rwlockattr_destroy ( &rwattr ) );
  state = 5;
  EC_RC ( pthread_mutexattr_destroy ( &mattr ) );
...
{
  char state = 0;
  EC_RC ( pthread_rwlock_destroy ( &lock->rwlock ) );
  state = 1;
  EC_RC ( pthread_mutex_destroy ( &lock->wlock ) );
  return 0;
...
{
  char state = 0;
  EC_RC ( pthread_mutex_lock ( &lock->wlock ) );
  state = 1;
  EC_RC ( pthread_rwlock_rwlock ( &lock->rwlock ) );
  state = 2;
  EC_RC ( pthread_mutex_unlock ( &lock->wlock ) );
...
int sqrp_rlock ( SQRPrwLock_t* lock )
{
  ...
  EC_RC ( pthread_mutex_lock ( &lock->wlock ) );
  state = 1;
  EC_RC ( pthread_rwlock_rdlock ( &lock->rwlock ) );
  state = 2;
  EC_RC ( pthread_mutex_unlock ( &lock->wlock ) );
...
/*!
  @short освобождает блокировку взятую на чтение/запись.
...
{
  EC_RC ( pthread_rwlock_unlock ( &lock->rwlock ) );
...
typedef struct {
  ...
  int32_t head; //!< голова - индекс позиции в массиве куда ведется запись новых сигналов
...
int sqrp_q_init ( size_t num_queues, size_t queue_len )
{
  ...
  size_t q_size = sizeof ( SQRPqsSegment_t ) + ( num_queues ) * sizeof ( SQRPqueue_t ) + num_queues * queue_len * sizeof ( SQPsignal_t );
  EC_NEG1 ( ftruncate ( qs_segfd, q_size ) );
  EC_CHCK0 ( qs_seg = mmap ( 0, q_size, PROT_READ | PROT_WRITE, MAP_SHARED, qs_segfd,
                             0 ), MAP_FAILED, == );
  state = 2;
  ...
  qs_seg->q = ( ( void* ) qs_seg ) + sizeof ( SQRPqsSegment_t );

  size_t ready_queues = 0;
  for ( int i = 0; i < num_queues; ++i ) {
    EC_NEG1 ( sqrp_lock_init ( &qs_seg->q[i].lock ) );
    qs_seg->q[i].code = UINT32_MAX;
    qs_seg->q[i].pass = 0;
    qs_seg->q[i].head = 0;
    qs_seg->q[i].data = ( ( void* ) qs_seg->q ) + ( num_queues ) * sizeof ( SQRPqueue_t ) + i * queue_len * sizeof ( SQPsignal_t );
    qs_seg->q[i].data[0].trust = 1;
    ++ready_queues;
  }
  close ( qs_segfd );
  ...
  EC_CLEAN_BEG {
    ...
    {
      case 2:
        for ( int i = 0; i < ready_queues; i++ )
          sqrp_lock_destroy ( &qs_seg->q[i].lock );
...
void sqrp_q_destroy ( void )
{
  ...
  for ( int i = 0; i < qs_seg->num_queues; i++ )
    sqrp_lock_destroy ( &qs_seg->q[i].lock );
...
int sqrp_q_attach ( void )
{
  char state = 0;
  int qs_segfd;
  EC_NEG1 ( qs_segfd = shm_open ( "/sq_rail.shm", O_RDWR, S_IRWXU ) );
  state = 1;

  EC_CHCK0 ( qs_seg = mmap ( 0, sizeof ( SQRPqsSegment_t ), PROT_READ | PROT_WRITE, MAP_SHARED, qs_segfd,
                             0 ), MAP_FAILED, == );
  size_t real_size = qs_seg->size;
  EC_NEG1 ( munmap ( qs_seg, sizeof ( SQRPqsSegment_t ) ) );
  EC_CHCK0 ( qs_seg = mmap ( 0, real_size, PROT_READ | PROT_WRITE, MAP_SHARED, qs_segfd,
                             0 ), MAP_FAILED, == );
  ...
  qs_seg->q = ( ( void* ) qs_seg ) + sizeof ( SQRPqsSegment_t );
  for ( int i = 0; i < qs_seg->num_queues; ++i )
    qs_seg->q[i].data = ( ( void* ) qs_seg->q ) + ( qs_seg->num_queues ) * sizeof ( SQRPqueue_t ) + i * qs_seg->queue_len * sizeof ( SQPsignal_t );
  return 0;
...
*/
static inline
int sqrp_q_set_code ( uint32_t qid, uint32_t code )
{
  if ( qid > qs_seg->queue_len - 1 ) {
    errno = EINVAL;
    return -1;
  }
  qs_seg->q[qid].code = code;
  return 0;
}
...
{
  for ( int i = 0; i < qs_seg->num_queues; i++ )
    if ( qs_seg->q[i].code == code ) return i;
...
{
  uint32_t pass = qs_seg->q[qid].pass;
  uint32_t head = qs_seg->q[qid].head;
...
void* sqrp_new_cursor ( uint32_t qid )
{
  ...
  EC_NULL ( cursor = malloc ( sizeof ( SQRPqueueCursor_t ) ) );
...
{
  EC_NEG1 ( sqrp_rlock ( &qs_seg->q[qid].lock ) );

  if ( cursor->compute_index ( qid, cursor ) == 0 ) {
    EC_NEG1 ( sqrp_unlock ( &qs_seg->q[qid].lock ) );
    return 0;
  }

  memcpy ( sig, &qs_seg->q[qid].data[cursor->tail], sizeof ( SQPsignal_t ) );
...
{
  EC_NEG1 ( sqrp_wlock ( &qs_seg->q[qid].lock ) );

  if ( qs_seg->q[qid].head == qs_seg->queue_len - 1 ) {
    qs_seg->q[qid].head = 0;
    ++ ( qs_seg->q[qid].pass );
  } else {
    ++ ( qs_seg->q[qid].head );
  ...
  memcpy ( &qs_seg->q[qid].data[qs_seg->q[qid].head], sig, sizeof ( *sig ) );
...
int sqrp_mb_acquire ( const char* name, size_t* size_p, SQRmbCreateFlag_t fl )
{
  ...
  char if_new = 0;
  int mbfd = shm_open ( name, O_RDWR, S_IRWXU );
  if ( mbfd == -1 ) {
    if ( errno != ENOENT ) EC_FAIL;
  ...
  if ( if_new ) {
    ...
    state = 3;
    mb_list.blocks[mb_list.len].owned = 1;
  } else {
    mb_list.blocks[mb_list.len].owned = 0;
  }

  // pthread_mutex_lock (&mb_list.lock);
  EC_REALLOC ( mb_list.blocks, ( mb_list.len + 1 ) * sizeof ( SQRPmbSegment_t ) );

  EC_NULL ( mb_list.blocks[mb_list.len].name = strdup ( name ) );
  mb_list.blocks[mb_list.len].lock = ptr;
  mb_list.blocks[mb_list.len].len = *size_p;
  mb_list.blocks[mb_list.len].valid = ptr + sizeof ( SQRPrwLock_t );
  *mb_list.blocks[mb_list.len].valid = 1;
  mb_list.blocks[mb_list.len].ptr = ( ( void* ) mb_list.blocks[mb_list.len].valid ) + 1;
  ++mb_list.len;
                   ...
                   SQRmbIoFlag_t fl )
{
  ...
  memcpy ( buf, mb_list.blocks[id].ptr + offset, len );
...
*/
static inline
int sqrp_mb_write ( int id, size_t offset, void* const buf, size_t len,
                    SQRmbIoFlag_t fl )
{
  if ( id >= mb_list.len || offset + len > mb_list.blocks[id].len || *mb_list.blocks[id].valid ) {
    errno = EINVAL;
  ...
  memcpy ( mb_list.blocks[id].ptr + offset, buf, len );
